...
let cityCount = localStorage.getItem("selectedDistrictsIstanbul")
  ? JSON.parse(localStorage.getItem("selectedDistrictsIstanbul")).length
  : 0;
document.getElementById("city_count").innerHTML = cityCount;

d3.json("istanbul-districts.json").then(function (data) {
  let width = 900;
  let height = 600;
  ...
  let g = svg
    ...
    .attr("fill", function (d, i) {
      if (localStorage.getItem("selectedDistrictsIstanbul")) {
        if (
          JSON.parse(localStorage.getItem("selectedDistrictsIstanbul")).includes(
      ...
      if (!d.noFill) {
        ...
        if (localStorage.getItem("selectedDistrictsIstanbul")) {
          let tempSelectedCities = JSON.parse(
            localStorage.getItem("selectedDistrictsIstanbul")
          );
          if (tempSelectedCities.includes(distName)) return;
          tempSelectedCities.push(distName);
          localStorage.setItem(
            "selectedDistrictsIstanbul",
            JSON.stringify(tempSelectedCities)
          );
        } else {
          let tempArr = [];
          tempArr.push(distName);
          localStorage.setItem("selectedDistrictsIstanbul", JSON.stringify(tempArr));
        }
      } else {
        cityCount--;
        document.getElementById("city_count").innerHTML = cityCount;
...
function downloadMap() {
  ...
  html2canvas(div).then(function (canvas) {
    ...
    ctx.fillStyle = "black";
    ctx.textAlign = "start";
    var textWidth = ctx.measureText("CityVisited/IstanbulVisited");
    ctx.fillText("IstanbulVisited", 10, canvas.height - 25);
    ctx.fillText(cityCount + "/39", 10, 5);

    destCanvas.toBlob(function (blob) {
      saveAs(blob, "istanbulvisited.png");
    });
  });
}

function resetButton() {
  localStorage.removeItem("selectedDistrictsIstanbul")
